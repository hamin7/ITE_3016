package ch14.uniformshop.shopping;
    ...
    public void insertBuy( List<CartDataBean> lists,
        ...
        try {
            ...
            for(int i=0; i<lists.size();i++){
            	...
            	sql = "insert into buy (buy_id,buyer,uniform_id,uniform_title,buy_price,buy_count,";
                sql += "uniform_image,buy_date,account,deliveryName,deliveryTel,deliveryAddress)";
                sql += " values (?,?,?,?,?,?,?,?,?,?,?,?)";
                pstmt = conn.prepareStatement(sql);
            
                pstmt.setLong(1, buyId);
                pstmt.setString(2, id);
                pstmt.setInt(3, cart.getUniform_id());
                pstmt.setString(4, cart.getUniform_title());
                pstmt.setInt(5, cart.getBuy_price());
                pstmt.setByte(6, cart.getBuy_count());
                pstmt.setString(7, cart.getUniform_image());
                ...
                //상품이 구매되었으므로 uniform테이블의 상품수량을 재조정함
                pstmt = conn.prepareStatement(
                		"select uniform_count from uniform where uniform_id=?");
                pstmt.setInt(1, cart.getUniform_id());
                rs = pstmt.executeQuery();
                rs.next();
                
                nowCount = (short)(rs.getShort(1) - cart.getBuy_count());
                
                sql = "update uniform set uniform_count=? where uniform_id=?";
                pstmt = conn.prepareStatement(sql);
           
                pstmt.setShort(1, nowCount);
    			pstmt.setInt(2, cart.getUniform_id());
        ...
        try {
            ...
            while (rs.next()) {
              buy = new BuyDataBean();
           	  ...
           	  buy.setUniform_id(rs.getInt("uniform_id"));
           	  buy.setUniform_title(rs.getString("uniform_title"));
           	  buy.setBuy_price(rs.getInt("buy_price"));
           	  buy.setBuy_count(rs.getByte("buy_count")); 
           	  buy.setUniform_image(rs.getString("uniform_image"));
